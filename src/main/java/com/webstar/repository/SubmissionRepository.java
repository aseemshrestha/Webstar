...
public interface SubmissionRepository extends JpaRepository<UserSubmissions, Long>
{
    ...
    @Query( value = "select * from User_Submissions u where u.is_active_post=1 and u.user_id = ?1 order by u.submittied_date desc limit ?2 offset ?3", nativeQuery = true )
    Optional<List<UserSubmissions>>findPostsByUserId(Long id, int limit, int offset); 
    
    @Query( value = "select * from User_Submissions u where u.is_active_post=1 and u.username = ?1 order by u.submittied_date desc limit ?2 offset ?3", nativeQuery = true )
